...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = [`${client.guilds.size} VIP Bot™`,'VIP Bot™ | -help',`${client.users.size} Members`,'VIP BOT | -invite'];
    var i = -1;
    var j = 0;
...
client.on("message", message => {
 if (message.content === "-invite") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setFooter('© VIP Bot :heart: جميع الحقوق محفوظة 2018 لــبوت')
      .addField('شكراخدامك لم فيايبي بوت', `https://modest-lewin-146a75.netlify.com`)
...
client.on("message", message => {
 if (message.content === "-support") {
  const embed = new Discord.RichEmbed()
      ...
      .setColor("RANDOM")
      .setFooter('© VIP Bot :heart: جميع الحقوق محفوظة 2018 لــبوت')  
      .addField('سيرفر الدعم الفني', ` https://discord.gg/Yx35DB8  `)
  message.author.send({embed});
...
client.on('message', message => {
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .addField('``VIP Bot اسمي هوا📇``' , `[ ${client.user.tag} ]` , true)
...
console.log(` VIP Bot  Added To Server ${guild.name} , The Owner Is ${guild.owner.user.username} , Members In Server : **[${guild.memberCount}]**            `)
client.channels.get("489944931868082177").send('** :purple_heart: VIP Bot ** ``Added``:white_check_mark:  To Server '+`** [ ${guild.name} ] **`+''+'  The Owner Is  ' +`**[ ${guild.owner.user.username} ]** , Members In Server : **[${guild.memberCount}]**` +'')
});

client.on("guildDelete", guild => {
  console.log(`  VIP Bot  Leave From Server ${guild.name}, The Server Owner Is ${guild.owner.user.username}`)
  client.channels.get("489944931868082177").send('** :purple_heart: VIP Bot  **``Kicked``:x:  From Server '+`** [ ${guild.name} ] **`+''+' The Owner Is ' +`**[ ${guild.owner.user.username} ]**` +'')
  });
